...
4. **Python version:** **3.11** — set in app **Settings → Advanced settings** (or at create time). Community Cloud **does not** read `runtime.txt`; if logs show Python 3.14, deps will fail to build.
...
| `Data file not found` | Ensure `outputs/*.csv` exist locally, or CSVs at repo root (app tries both paths) |
| Pillow/pandas build fails on Streamlit Cloud | Set **Python 3.11** in app Settings (not 3.14); reboot |
